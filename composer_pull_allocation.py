...
def api_request(cfg: dict, method: str, path: str, body: dict | None = None) -> dict:
    ...
    headers = {
        ...
        # Browser-like UA + headers to avoid Cloudflare Error 1010 (browser_signature_banned)
...
def main() -> int:
    ...
    def _acct_uuid(a):
        return a.get("account_uuid") or a.get("uuid") or a.get("id")

    account_uuids: list = []
    if cfg.get("account_uuid"):
        account_uuids = [cfg.get("account_uuid")]
    else:
        want_nums = cfg.get("account_numbers")
        if not want_nums:
            _single = str(cfg.get("account_number", "")).strip()
            want_nums = [_single] if _single else []
        want_nums = [str(n).strip() for n in want_nums if str(n).strip()]
        ...
        if not accts:
            ...
            return 1
        for a in accts:
            log("  account: uuid=" + str(_acct_uuid(a)) + "  " + json.dumps(a, default=str)[:240])
        if want_nums:
            for num in want_nums:
                hit = next((a for a in accts if num in json.dumps(a, default=str)), None)
                if hit:
                    account_uuids.append(_acct_uuid(hit))
                    log("Matched account_number " + num + " -> " + str(_acct_uuid(hit)))
                else:
                    log("WARN: account_number " + num + " not found among accounts")
        if not account_uuids:
            account_uuids = [_acct_uuid(accts[0])]
            log("Using first account: " + str(account_uuids[0]))

    account_uuid = account_uuids[0]
    log("Pulling from " + str(len(account_uuids)) + " account(s): " + str(account_uuids))

    today = dt.date.today().isoformat()

    # Fetch symphony-stats-meta for EACH account and MERGE, so configured
    # symphonies are found regardless of which account holds them.
    log("Fetching per-symphony stats from /portfolio/.../symphony-stats-meta...")
    stats_payload = None
    _merged_syms: list = []
    _seen_ids = set()
    for _au in account_uuids:
        _sp = get_symphony_stats(cfg, _au)
        if _sp is None:
            continue
        if stats_payload is None:
            stats_payload = dict(_sp)
        for _s in (_sp.get("symphonies") or []):
            _sid2 = _s.get("id") or _s.get("symphony_id")
            if _sid2 and _sid2 in _seen_ids:
                continue
            if _sid2:
                _seen_ids.add(_sid2)
            _merged_syms.append(_s)
    if stats_payload is not None:
        stats_payload["symphonies"] = _merged_syms
        stats_path = SCRIPT_DIR / f"composer_symphony_stats_{today}.json"
        with open(stats_path, "w") as f:
            json.dump(stats_payload, f, indent=2, default=str)
        log("Saved merged symphony-stats (" + str(len(_merged_syms)) + " symphonies) to " + stats_path.name)
    ...
    for sym in symphonies:
        ...
        # Only runs for Algorithm 185 symphony (ZGersOlsQFRHg7U2qnSw — re-pointed from dead qjm 2026-06-23)
        # Source CSV: composer_allocations_185_3yr2.csv  (10 ticker cols, no SPXU/VIXY)
        if sid == "ZGersOlsQFRHg7U2qnSw":
            try:
